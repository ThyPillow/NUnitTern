...
using System.Linq;
using static NUnitTern.Utils.SyntaxHelper;

namespace NUnitTern.Utils
{
    internal class IgnoreRelatedAttributeManager
    {
        private const string DefaultReasonMessageFix = @"""Automatically added empty reason during NUnit 3 migration""";
        ...
        public IgnoreRelatedAttributeManager(AttributeSyntax attribute)
        {
            Attribute = attribute;
            ...
            if (Kind == IgnoreKind.IgnoreAttribute)
                ParseAttributeArguments(attribute, ParseIgnoreAttributeArgs);
            else
                ParseAttributeArguments(attribute, ParseTestCaseOrFixtureArgs);
        }
